...
**There is an error in the 'running PNL' I am working on.**
...
todayPricesK <- DATAK %>% select(-close) %>% pivot_wider(names_from = symbol, values_from = closeSpot) %>% arrange(Date) %>% last()
###########
# New Crop
...
DATANew <- DATA %>% mutate (closeSpot = case_when(symbol == "ZC" ~ close + (.05*exp(close/100 - 450/100) - 25/100)*100,
                                       symbol == "ZS" ~ close + (.05*exp(close/100 - 1260/100) - 50/100)*100,
                                       symbol == "ZW" ~ close + (.05*exp(close/100 - 600/100) - 25/100)*100))
todayPricesNew <- DATANew %>% select(-close) %>% pivot_wider(names_from = symbol, values_from = closeSpot) %>% arrange(Date) %>% last()

```

```{r}
#March cprn Calculations
tableZCK <- left_join(my_sheet,  
 ...
 #                                      `Corn K Running Positions` != 0 ~ k_corn_notional + `K Corn`* k_corn_lastPrice )) %>% 
  mutate(k_corn_runningPNL = round(case_when(k_corn_runningPNL != 0 ~ (-1)*k_corn_runningPNL/100 + `K Corn`*k_corn_lastPrice/100,
                                       TRUE ~ 0), 2 ))  %>%
...
tableZCZ <- left_join(my_sheet,  
 ...
 # mutate(k_corn_notional = case_when(`Corn K Running Positions` == 0 ~ k_corn_notional,
 #                                      `Corn K Running Positions` != 0 ~ k_corn_notional + `K Corn`* k_corn_lastPrice )) %>% 
  mutate(z_corn_runningPNL = round(case_when(z_corn_runningPNL != 0 ~ (-1)*z_corn_runningPNL/100 + `Z Corn`*z_corn_lastPrice/100,
                                       TRUE ~ 0), 2 ))  %>%
...
tableZSK <- left_join(my_sheet,  
  ...
  mutate(k_soybean_runningPNL = round(case_when(k_soybean_runningPNL != 0 ~ (-1)*k_soybean_runningPNL/100 + `K Soybean`*k_soybean_lastPrice/100,
                                       TRUE ~ 0), 2 ))  %>%
